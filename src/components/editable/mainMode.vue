<template>
    <div>
        <el-row :span="8" v-for="(o, index) in flag.length/2" :key="o" >
            <el-col v-for="(o, i) in 2" :key="o" :offset="i > 0 ? 2 : 0" style="width: 40%">
                <el-button-group>
                    <el-button @click="replace(index*2 + i)" size="small">更换视图</el-button>
                    <el-button @click="del(index*2 + i)" size="small">删除视图</el-button>
                    <el-button @click="replace(index*2 + i)" size="small">选择数据</el-button>
                </el-button-group>
                <div>
                    <component :is="flag[index*2 + i]"></component>
                </div>
            </el-col>
        </el-row>
...
<script>
    ...
    import CvTreeGrid from '../../views/editable/CvTreeGrid'
    export default {
        name: "mainMode",
        data(){
            return {
            ...
            requestData(){
                ...
                if (showType === "2DLine"){
                    ...
                    if (selection === "用户组"){
                        ...
                        this.$set(this.flag, this.temp, CvTreeGrid);
                    }
                }
                this.$store.commit("SET_FLAG_STATE",this.flag)
            }

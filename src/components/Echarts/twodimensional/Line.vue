<template>
    <div>
        ...
        <div ref="line_dv" style="width: auto; height: 300px"></div>
        </el-card>
    </div>
</template>
<script>
    ...
    export default {
        ...
        mounted(){
            this.matchLine(this.$store.state.selection);
        },
        methods: {
            drawLine() {
                // 基于准备好的dom，初始化echarts实例
                let myChart = echarts.init(this.$refs.line_dv);
                // 绘制图表
            ...
            matchLine(selection){
                ...
                if (selection === "用户组") {
                    fetchTree().then(res => {
                        ...
                        // console.log("x:" + that.dataSet.xAxis.data);
                        // console.log("s:" + that.dataSet.series[0].data);
                        that.$nextTick(()=>{
                            that.drawLine();
                        })
                    });
